...
hessian0 <- function(par, rdata, edata, omega, par.pos){
  ...
  if(1){
  hess['c', 'c'] <- -ne/c^2/2 - bet.z^2/c * sum(ed)
  hess['c', 'alp.0'] <- -bet.z/c * sum(ed)

  if(nx > 0){
    if(m31 > 0){
      hess['c', name.alp.x] <- -bet.z/c * (m31 * t(t(rx) %*% p1))
    }

    if(m21 > 0){
      hess['c', name.alp.x] <- hess['c', name.alp.x] - bet.z/c * t(t(ex21) %*% one21)
    }
  }

  if(nh > 0){
    if(m31 > 0){
      hess['c', name.alp.h] <- -bet.z/c * t(t(eh31) %*% one31)
    }

    if(m21 > 0){
      hess['c', name.alp.h] <- hess['c', name.alp.h] - bet.z/c * t(t(eh21) %*% one21)
    }
  }

  if(m31 > 0){
    hess['c', name.alp.g] <- -bet.z/c * (m31 * t(t(rg) %*% p1))
  }

  if(m21 > 0){
    hess['c', name.alp.g] <- hess['c', name.alp.g] - bet.z/c * t(t(eg21) %*% one21)
  }

  if(est.b){
    hess['c', 'b'] <- -bet.z/c * sum(ed)
  }

  hess['c', name.bet.z] <- -bet.z * sum(ed)
  }
  ...
  if(nh > 0){
    if(m3 > 0){
      hess['alp.0', name.alp.h] <- -1/c * t(t(eh3) %*% one3)
    }

    if(m2 > 0){
      hess['alp.0', name.alp.h] <- hess['alp.0', name.alp.h] - 1/c * t(t(eh2) %*% one2)
    }
  }
  ...
  if(nx > 0){
    ...
    if(m31 > 0){
      hess[name.alp.x, name.bet.z] <- - m31 * (t(rx) %*% p1)
    }
    if(m21 > 0){
      hess[name.alp.x, name.bet.z] <- hess[name.alp.x, name.bet.z] - t(ex21) %*% one21
    }
  }
  ...
  if(nh > 0){
    ...
    if(m31 > 0){
      hess[name.alp.h, name.bet.z] <- - (t(eh31) %*% one31)
    }
    if(m21 > 0){
      hess[name.alp.h, name.bet.z] <- hess[name.alp.h, name.bet.z] - (t(eh21) %*% one21)
    }
  }

  ############

  a33 <- -bet.z^2 * n0 * (t(rg) %*% (p0 * Delta * rg))
  hess[name.alp.g, name.alp.g] <- -a33

  if(m30 > 0){
    hess[name.alp.g, name.alp.g] <- hess[name.alp.g, name.alp.g] - 1/c * (m30 * (t(p0 * rg) %*% rg))
  }
  if(m31 > 0){
  ...
  hess[name.alp.g, 'a'] <- bet.z * n0 * (t(rg) %*% (p0 * Delta))

  if(nx > 0){
    hess[name.alp.g, name.bet.x] <- bet.z * n0 * (t(rg) %*% (p0 * Delta * rx))
  }

  if(ny > 0){
    hess[name.alp.g, name.bet.y] <- bet.z * n0 * (t(rg) %*% (p0 * Delta * ry))
  }

  hess[name.alp.g, name.bet.z] <- bet.z * n0 * (t(rg) %*% (p0 * Delta * eta))
  if(m31 > 0){
    hess[name.alp.g, name.bet.z] <- hess[name.alp.g, name.bet.z] - (m31 * (t(rg) %*% p1))
  }
  if(m21 > 0){
    hess[name.alp.g, name.bet.z] <- hess[name.alp.g, name.bet.z] - (t(eg21) %*% one21)
  }
  ...
  hess[name.bet.z, name.bet.z] <- n0 * sum(p0 * Delta * eta^2) - c * sum(ed)
